...
export function ProductListMatrix({ productCode, queryDate, currentListCode }: { productCode: string; queryDate: string; currentListCode: string }) {
  ...
  if (!data) return <p className="matrixEmpty">Sin datos de otras listas.</p>;

  const priced = data.lists.filter((l) => l.has_price);
  if (priced.length === 0) return <p className="matrixEmpty">Este producto no tiene precio vigente en ninguna lista.</p>;

  return (
    <div className="listMatrix">
      <p className="matrixLead">Tarifado en {priced.length} de {data.lists.length} listas de precio</p>
      <div className="matrixGrid">
        {priced.map((cell) => (
          <div
            key={cell.code}
            className={`matrixCell ${cell.thermometer} ${cell.code === currentListCode ? "current" : ""}`}
            title={`${cell.description}: margen ${formatPercent(cell.gain_percent)}${cell.gap_points !== null ? ` (${cell.gap_points > 0 ? "+" : ""}${cell.gap_points} pp vs objetivo)` : ""}`}
          >
